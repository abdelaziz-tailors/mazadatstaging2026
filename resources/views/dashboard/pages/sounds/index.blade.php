...
@section('title') {{ TranslationHelper::translate('Sounds') }} @endsection
...
<div class="page-header">
    <div class="row">
        <div class="col">
            <h3 class="page-title" >
                {{TranslationHelper::translate('Sounds') }}
                @if(Auth::guard('admin')->user()->can('add sound'))
                    <a href='{{ route('admin.sounds.create') }}' class='btn btn-primary float-end'><i class="fas fa-plus"></i> {{ TranslationHelper::translate('Sound') }}</a>
                @endif
            </h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard.index')}}">{{ TranslationHelper::translate('dashboard') }}</a></li>

                <li class="breadcrumb-item active">{{ TranslationHelper::translate('Sound') }}</li>
            </ul>
        </div>
    </div>
</div>
            ...
            <table id="data-table" class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th>{{ TranslationHelper::translate('Sound') }}</th>
...
<script>
    $('#data-table').DataTable({
        ...
        ajax: {
            url : "{!! route('admin.sounds.getData') !!}",
            data: {},
            type: "POST",
            dataType: "JSON"
        },
        columns: [
            ...
            {data: 'sound', 'searchable': true, 'orderable': true, 'exportable': true, 'printable': true},
