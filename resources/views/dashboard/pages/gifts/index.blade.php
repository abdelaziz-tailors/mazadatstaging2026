...
@section('title') {{ TranslationHelper::translate('Gifts') }} @endsection
...
<div class="page-header">
    <div class="row">
        <div class="col">
            <h3 class="page-title" >
                {{TranslationHelper::translate('Gifts') }}
                @if(Auth::guard('admin')->user()->can('add gift'))
                    <a href='{{ route('admin.gifts.create') }}' class='btn btn-primary float-end'><i class="fas fa-plus"></i> {{ TranslationHelper::translate('Gift') }}</a>
                @endif
            </h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard.index')}}">{{ TranslationHelper::translate('dashboard') }}</a></li>

                <li class="breadcrumb-item active">{{ TranslationHelper::translate('Gift') }}</li>
            </ul>
        </div>
    </div>
</div>
