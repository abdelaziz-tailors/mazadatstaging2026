...
<div class="page-header">
    <div class="row">
        <div class="col">
            <h3 class="page-title" >
                {{ TranslationHelper::translate('age') }}
{{--                @if(Auth::guard('admin')->user()->can('add age'))--}}
                    <a href='{{ route('admin.ages.create') }}' class='btn btn-primary float-end'><i class="fas fa-plus"></i> {{ TranslationHelper::translate('new age') }}</a>
{{--                @endif--}}
            </h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard.index')}}">{{ TranslationHelper::translate('dashboard') }}</a></li>
                <li class="breadcrumb-item active">{{ TranslationHelper::translate('ages') }}</li>
            </ul>
        </div>
    </div>
</div>
...
<script>
    $('#data-table').DataTable({
        ...
        language: {
            ...
            "searchPlaceholder": "{{ TranslationHelper::translate('search_ages_placeholder') }}",
