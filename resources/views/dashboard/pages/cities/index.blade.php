...
<div class="page-header">
    <div class="row">
        <div class="col">
            <h3 class="page-title" >
                {{ TranslationHelper::translate('cities_of') }}
{{--                @if(Auth::guard('admin')->user()->can('add city'))--}}
                    <a href='{{ route('admin.cities.create') }}' class='btn btn-primary float-end'><i class="fas fa-plus"></i> {{ TranslationHelper::translate('new_city') }}</a>
{{--                @endif--}}
            </h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard.index')}}">{{ TranslationHelper::translate('dashboard') }}</a></li>
                <li class="breadcrumb-item active">{{ TranslationHelper::translate('Cities') }}</li>
            </ul>
        </div>
    </div>
</div>
...
<script>
    $('#data-table').DataTable({
        ...
        ajax: {
            url : '{!! route("admin.cities.getData") !!}',
            type: "POST",
            dataType: "JSON"
        },
        columns: [
