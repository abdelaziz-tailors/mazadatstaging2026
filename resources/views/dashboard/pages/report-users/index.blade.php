...
@section('title') {{ TranslationHelper::translate('User Report') }} @endsection
...
<div class="page-header">
    <div class="row">
        <div class="col">
            <h3 class="page-title" >
                {{TranslationHelper::translate('Reports') }}
                @if(Auth::guard('admin')->user()->can('add user_report'))
                    <a href='{{ route('admin.report-users.create') }}' class='btn btn-primary float-end'><i class="fas fa-plus"></i> {{ TranslationHelper::translate('User Report') }}</a>
                @endif
            </h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard.index')}}">{{ TranslationHelper::translate('dashboard') }}</a></li>

                <li class="breadcrumb-item active">{{ TranslationHelper::translate('User Report') }}</li>
            </ul>
        </div>
    </div>
</div>
...
<script>
    $('#data-table').DataTable({
        ...
        ajax: {
            url : "{!! route('admin.report.users.getData') !!}",
            data: {},
            type: "POST",
            dataType: "JSON"
        },
