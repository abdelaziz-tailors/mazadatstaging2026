...
<div class="row">
    @if(isset($gift))
        @php
            ...
            $artist_names = json_decode($gift->artist_name, true);

        @endphp
    @else
        @php $names = []; @endphp
        @php $artist_names = []; @endphp
            ...
            <div class='card-body'>
                <div class="tab-content pt-0">
                    ...
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <div role="tabpanel" id="tab-{{ $localeCode }}" class="tab-pane fade @if($loop->index == 0) show active @endif "
                            ...
                            <div class="col-lg-6 form-group">
                                {!! Form::label('artist_name['.$localeCode.']', TranslationHelper::translate('Artist Name', $localeCode), ['class'=>'form-label']) !!}
                                {!! Form::text('artist_name['.$localeCode.']', (is_array($artist_names) && array_key_exists($localeCode, $artist_names)) ? $artist_names[$localeCode] : NULL, ['class' => 'form-control']) !!}
                            </div>
                        </div>
                    @endforeach
                </div>
                <div class="row">
                    ...
                    <div class="form-group @if (isset($gift)) col-lg-5 @else col-lg-6 @endif ">
                        {!! Form::label('sound', TranslationHelper::translate('Sound'), ['class' => 'form-label']) !!}
                        <input type="file" id="sound" name="sound" class="form-control" />
                    </div>
                    @if (isset($gift))
                        <div class="form-group col-lg-6">

                            <audio controls>
                                <source src="{{ Storage::disk('public')->url($gift->sound) }}" type="audio/ogg">
                                <source src="{{ Storage::disk('public')->url($gift->sound) }}" type="audio/mpeg">
                    ...
                    <div class="form-group @if (isset($gift)) col-lg-5 @else col-lg-6 @endif ">
                        {!! Form::label('sound', TranslationHelper::translate('Image'), ['class' => 'form-label']) !!}
                        <input type="file" id="image" name="image" class="form-control" />
                    </div>
                    @if (isset($gift))
                        <div class="form-group col-lg-1">
                            @if (Storage::disk('public')->exists($gift->image))
                                <img src="{{ Storage::disk('public')->url($gift->image) }}" class="img-fluid" />
                            @else
                            @endif
                        </div>
                    @endif
