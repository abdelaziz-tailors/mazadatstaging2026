<div class="btn-group">
    ...
    <div class="dropdown-menu">
        @if (Auth::guard('admin')->user()->can('edit gift'))
            <a class="dropdown-item" href="{{ route('admin.gifts.edit', $item->id) }}">
                <i class="fas fa-edit"></i> {{ TranslationHelper::translate('edit') }}
            </a>
        @endif

    @if (Auth::guard('admin')->user()->can('delete gift'))
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticBackdropLabel">{{ TranslationHelper::translate('Delete gift') }}</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <form role="form" action="{{ url('/admin/gifts/'.$item->id) }}" class="" method="POST">
