...
@section('title') {{ TranslationHelper::translate('Notifications') }} @endsection
...
<div class="page-header">
    <div class="row">
        <div class="col">
            <h3 class="page-title" >
                {{ TranslationHelper::translate('Notifications') }}
                @if(Auth::guard('admin')->user()->can('add notification'))
                    <a href='{{ route('admin.notifications.create') }}' class='btn btn-primary float-end'><i class="fas fa-plus"></i> {{ TranslationHelper::translate('new Notifications') }}</a>
                @endif
            </h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard.index')}}">{{ TranslationHelper::translate('dashboard') }}</a></li>
                <li class="breadcrumb-item active">{{ TranslationHelper::translate('Notifications') }}</li>
            </ul>
        </div>
    </div>
</div>
...
<script>
    $('#data-table').DataTable({
        ...
        ajax: {
            url : "{!! route('admin.notifications.getData') !!}",
            type: "POST",
            dataType: "JSON"
        },
        columns: [
            {data: 'id', 'searchable': true, 'orderable': true, 'exportable': true, 'printable': true},
            {data: 'title', 'searchable': true, 'orderable': true, 'exportable': true, 'printable': true},
            {data: 'description', 'searchable': true, 'orderable': false, 'exportable': true, 'printable': true},
        ...
        language: {
            ...
            "searchPlaceholder": "{{ TranslationHelper::translate('search_notifications_placeholder') }}",
