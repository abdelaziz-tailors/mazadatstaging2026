...
{{--    @if (Auth::guard('admin')->user()->can('edit animal_pen'))--}}
        <a class="dropdown-item" href="{{ route('admin.animal-pens.edit', $item->id) }}">
          <i class="fas fa-edit"></i> {{ TranslationHelper::translate('edit') }}
        </a>
{{--    @endif--}}


{{--    @if (Auth::guard('admin')->user()->can('delete animal_pen'))--}}
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticBackdropLabel">{{ TranslationHelper::translate('delete animal pen') }}</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <form role="form" action="{{ url('/admin/animal-pens/'.$item->id) }}" class="" method="POST">
