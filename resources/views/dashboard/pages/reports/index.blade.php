...
@section('title') {{ TranslationHelper::translate('Reports') }} @endsection
        ...
        <div class="col">
            <h3 class="page-title" >
                ...
                @if(Auth::guard('admin')->user()->can('add sound'))
                    <a href='{{ route('admin.reports.create') }}' class='btn btn-primary float-end'><i class="fas fa-plus"></i> {{ TranslationHelper::translate('Report') }}</a>
                @endif
            </h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard.index')}}">{{ TranslationHelper::translate('dashboard') }}</a></li>

                <li class="breadcrumb-item active">{{ TranslationHelper::translate('Report') }}</li>
            </ul>
        </div>
    </div>
</div>
